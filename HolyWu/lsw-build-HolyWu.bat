...
.\msys64\usr\bin\bash -lc "git clone --depth=1 https://chromium.googlesource.com/webm/libvpx && cd libvpx && mkdir vpx_build && cd vpx_build && ../configure --enable-vp9_highbitdepth --disable-vp8-encoder --disable-vp9-encoder --prefix=/mingw32/ && make -j %NUMBER_OF_PROCESSORS% && make install"
...
.\msys64\usr\bin\bash -lc "git clone --depth=1 https://code.videolan.org/videolan/dav1d && cd dav1d && mkdir dav1d_build && cd dav1d_build && meson ../ --buildtype release --default-library static && ninja && ninja install"
.\msys64\usr\bin\bash -lc "git clone --depth=1 -b n%FFmpeg_ver% git://source.ffmpeg.org/ffmpeg.git ffmpeg-%FFmpeg_patch_ver% && curl -O -L %FFmpeg_patch% && patch -p0 <ffmpeg-%FFmpeg_patch_ver%-allpatches.diff && cd ffmpeg-%FFmpeg_patch_ver% && ./configure --target-os=mingw32 --disable-shared --disable-debug --disable-encoders --disable-decoder=vp9 --disable-doc --enable-libdav1d --enable-libvpx --enable-libmfx --enable-static --enable-gpl --enable-version3 --enable-runtime-cpudetect --enable-avisynth && make -j %NUMBER_OF_PROCESSORS% && make install"
...
for /f "tokens=2" %%i in ('findstr /R /C:"^[0-9a-f][0-9a-f]* [0-9a-f][0-9a-f]*" ".\msys64\home\%USERNAME%\ffmpeg-%FFmpeg_patch_ver%\.git\logs\HEAD"') do echo FFmpeg ver %FFmpeg_ver% %%~i >>ver.txt
PowerShell Compress-Archive -Path "ver.txt,.\msys64\home\%USERNAME%\L-SMASH-Works\AviUtl\lwinput.aui,.\msys64\home\%USERNAME%\L-SMASH-Works\AviUtl\lwdumper.auf,.\msys64\home\%USERNAME%\L-SMASH-Works\AviUtl\lwmuxer.auf,.\msys64\home\%USERNAME%\L-SMASH-Works\AviUtl\lwcolor.auc" -DestinationPath L-SMASH-Works-HolyWu-r%revision%.zip 
