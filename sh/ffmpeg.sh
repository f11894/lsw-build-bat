...
if [ ! -d ffmpeg-${FFmpeg_patch_ver} ]; then
    git clone --depth=1 -b n${FFmpeg_ver} git://source.ffmpeg.org/ffmpeg.git ffmpeg-${FFmpeg_patch_ver}
    curl -O -L ${FFmpeg_patch}
    patch -p0 <ffmpeg-${FFmpeg_patch_ver}-allpatches.diff
fi
cd ffmpeg-${FFmpeg_patch_ver}
