...
.\msys64\usr\bin\bash -lc "git clone --depth=1 https://aomedia.googlesource.com/aom && mkdir aom_build && cd aom_build && cmake ../aom -G Ninja -DFORCE_HIGHBITDEPTH_DECODING=0 -DCONFIG_AV1_ENCODER=0 -DCMAKE_INSTALL_PREFIX=/mingw32/ -DCMAKE_BUILD_TYPE=Release && ninja -j %NUMBER_OF_PROCESSORS% && ninja install"
.\msys64\usr\bin\bash -lc "git clone --depth=1 -b n%FFmpeg_ver% git://source.ffmpeg.org/ffmpeg.git ffmpeg-%FFmpeg_ver% && curl -O -L %FFmpeg_patch% && patch -p0 <ffmpeg-%FFmpeg_ver%-allpatches.diff && cd ffmpeg-%FFmpeg_ver% && ./configure --target-os=mingw32 --disable-shared --disable-debug --disable-encoders --disable-doc --enable-avresample --enable-libaom --enable-libmfx --enable-static --enable-gpl --enable-version3 --enable-runtime-cpudetect --enable-avisynth && make -j %NUMBER_OF_PROCESSORS% && make install"
...
for /f "tokens=2" %%i in ('findstr /R /C:"^[0-9a-f][0-9a-f]* [0-9a-f][0-9a-f]*" ".\msys64\home\%USERNAME%\aom\.git\logs\HEAD"') do echo libaom ver %%~i >>ver.txt
...
.\7-Zip\Files\7-Zip\7z.exe a L-SMASH-Works-VFR-maniac-r%revision%.zip ver.txt .\msys64\home\%USERNAME%\L-SMASH-Works\AviUtl\lwinput.aui .\msys64\home\%USERNAME%\L-SMASH-Works\AviUtl\lwdumper.auf .\msys64\home\%USERNAME%\L-SMASH-Works\AviUtl\lwmuxer.auf .\msys64\home\%USERNAME%\L-SMASH-Works\AviUtl\lwcolor.auc
